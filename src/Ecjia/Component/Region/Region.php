...
class Region
{
    ...
    public function getProvinces()
    {
        ...
        return $this->database_connection->table('regions')->where('parent_id', $country)->select('region_id', 'region_name')->get();
    }
    ...
    {
        return $this->database_connection->table('regions')->where('parent_id', $regionId)->select('region_id', 'region_name', 'parent_id', 'region_type')->get();
    }
    ...
    {
        return $this->database_connection->table('regions')->where('region_id', $regionId)->select('region_id', 'region_name', 'parent_id', 'region_type')->first();
    }
    ...
    {
        return $this->database_connection->table('regions')->whereIn('region_id', $regionIds)->select('region_id', 'region_name', 'parent_id', 'region_type')->get();
    }
    ...
    {
        if ($type < 4) {
            $result = $this->database_connection->table('regions')->where('region_type', $type)->select('region_id', 'region_name', 'parent_id', 'region_type')->get();
        } else {
            $result = [];
        }
    ...
    {
        if (is_null($type)) {
            $result = $this->database_connection->table('regions')->where('region_name', 'like', '%' . ecjia_mysql_like_quote($name) . '%')->get();
        } else {
            $result = $this->database_connection->table('regions')->where('region_name', 'like', '%' . ecjia_mysql_like_quote($name) . '%')->where('region_type', $type)->get();
        }
        return $result;
    }
